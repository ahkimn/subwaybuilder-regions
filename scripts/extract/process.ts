...
export function processAndSaveBoundaries(
  ...
  if (!geoJson || !geoJson.features || geoJson.features.length === 0) {
    console.warn(`No features returned from source data for ${countryCode} ${dataConfig.displayName} within specified boundary box.`);
    return;
  }

  console.log(`Fetched ${geoJson.features.length} total features from source data for ${countryCode} ${dataConfig.displayName}.`);
