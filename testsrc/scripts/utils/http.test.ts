...
describe('scripts/utils/http fetchJsonWithRetry', () => {
  ...
  it('retries transient status errors (429/5xx) then succeeds', async () => {
    ...
    const result = await fetchJsonWithRetry('https://example.com/retry-status', undefined, {
      maxRetries: 3,
      retryDelayMs: 1,
      label: 'TEST',
    });
  ...
  it('does not retry non-transient status and throws formatted error', async () => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.match(error.message, /\[TEST\] GET https:\/\/example\.com\/non-transient failed on attempt 1\/5:/);
        assert.match(error.message, /failed with status 400 Bad Request \(attempt 1\/5\)/);
        return true;
      },
    );
  ...
  it('throws on non-JSON response and includes response snippet', async () => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.ok(error instanceof Error);
        assert.match(error.message, /returned non-JSON response \(text\/html\)/);
        assert.match(error.message, /Body starts with: <html><body>upstream proxy error<\/body><\/html>/);
        return true;
      },
    );
  });
  ...
  it('retries on network TypeError and then succeeds', async () => {
    ...
    const result = await fetchJsonWithRetry('https://example.com/network-retry', undefined, {
      maxRetries: 3,
      retryDelayMs: 1,
      label: 'TEST',
    });
  ...
  it('retries on AbortError and eventually fails with attempt metadata', async () => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.ok(error instanceof Error);
        assert.match(error.message, /\[TEST\] GET https:\/\/example\.com\/abort-retry failed on attempt 3\/3:/);
