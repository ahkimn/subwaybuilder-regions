...
function loadFixture(relativeFixturePath: string): string {
  return fs.readFileSync(path.join(__dirname, 'fixtures', relativeFixturePath), 'utf8');
}
...
describe('scripts/utils/files', () => {
  ...
  function createTempDirectory(): string {
    const tempDirectoryPath = fs.mkdtempSync(path.join(os.tmpdir(), 'files-test-'));
  ...
  it('saveGeoJSON_shouldWriteUncompressedAndCompressedFiles', () => {
    ...
    const compressedBasePath = path.join(outputDirectoryPath, 'regions-compressed.geojson');
    ...
    const loadedUncompressed = JSON.parse(fs.readFileSync(uncompressedPath, 'utf8'));
  ...
  it('loadGeoJSON_shouldLoadCompressedAndUncompressedFiles', () => {
    ...
    fs.writeFileSync(compressedPath, gzipSync(JSON.stringify(featureCollection)));
  ...
  it('loadGeoJSONFromNDJSON_shouldReadMultipleLineDelimitedFeatures', async () => {
    ...
    fs.writeFileSync(ndjsonPath, `${JSON.stringify(featureOne)}\n\n${JSON.stringify(featureTwo)}\n`);
  ...
  it('updateIndexJson_shouldCreateCityEntryFromFixtureIndex', () => {
    ...
    const index = JSON.parse(fs.readFileSync(indexPath, 'utf8')) as DatasetIndex;
  ...
  it('updateIndexJson_shouldUpdateExistingDatasetEntryFromFixtureIndex', () => {
    const tempDirectoryPath = createTempDirectory();
    const indexPath = path.join(tempDirectoryPath, 'data_index.json');
    fs.writeFileSync(indexPath, loadFixture('index-update-existing.fixture.json'));
